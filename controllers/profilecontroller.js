...
const changepasswords = async (req, res) => {
    ...
    try {
        ...
        if (!account) {
            return res.status(404).json({ message: "Error Occured no user found " })
        }
        ...
        if (newpassword !== confirmpassword) {
            return res.status(404).json({ message: "newpassword/confirmpassword mismatch" })
        }
...
const updateprofiles = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Your Profile has been changed! " })
    } catch (error) {
        console.log(error.message)
        return res.status(404).json({ message: "Error Occured!!" })
    }
